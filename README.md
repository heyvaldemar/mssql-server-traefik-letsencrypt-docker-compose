...
Every release from v1.6.2 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `mssql-server-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `mssql-server-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `mssql-server-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob mssql-server-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle mssql-server-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/mssql-server-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact mssql-server-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path mssql-server-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/mssql-server-traefik-letsencrypt-docker-compose
```
